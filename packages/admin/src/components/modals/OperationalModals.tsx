...
export function ProductFormModal({ open, product, onClose }: { open: boolean; product?: ProductRecord; onClose: () => void }) {
  ...
  const categoryOptions = useMemo(() => productCategories.filter((item) => item.status !== "Inativo").map((item) => item.name), [productCategories]);
  ...
  useEffect(() => {
    ...
    setDescription(product?.description ?? "Produto para venda no POS da unidade.");
  ...
  return (
    <Modal open={open} title={product ? "Editar produto" : "Novo produto"} description={product ? "Altere as informações do produto abaixo." : "Preencha as informações para cadastrar um novo produto."} size="lg" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>{product ? "Salvar alterações" : "Salvar produto"}</Button></>}>
      <div className="space-y-5">
        <Section title="1. Informacoes basicas"><div className="grid grid-cols-2 gap-3"><FormInput label="Nome do produto" requiredMark value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex: Whey Protein 900g" /><FormSelect label="Categoria" requiredMark options={categoryOptions.length ? categoryOptions : ["Suplementos"]} value={category} onChange={(event) => setCategory(event.target.value)} /><FormInput label="Codigo de barras" value={barcode} onChange={(event) => setBarcode(event.target.value)} /><FormInput label="SKU" value={sku} onChange={(event) => setSku(event.target.value)} /></div><FormTextarea label="Descricao" placeholder="Descreva o produto..." value={description} onChange={(event) => setDescription(event.target.value)} /></Section>
        <Section title="2. Preco e estoque"><div className="grid grid-cols-4 gap-3"><FormInput label="Preco de venda (Kz)" requiredMark type="number" min="0" value={price} onChange={(event) => setPrice(event.target.value)} /><FormInput label="Preco de custo (Kz)" requiredMark type="number" min="0" value={cost} onChange={(event) => setCost(event.target.value)} /><FormInput label="Estoque atual" requiredMark type="number" min="0" value={stock} onChange={(event) => setStock(event.target.value)} /><FormSelect label="Unidade" requiredMark options={["Unidade", "Caixa", "Pacote", "Kg", "Litro"]} value={unit} onChange={(event) => setUnit(event.target.value)} /></div><div className="grid grid-cols-2 gap-3"><FormSwitch label="Controlar estoque" checked={controlStock} onChange={setControlStock} /><FormInput label="Estoque minimo" type="number" min="0" value={minStock} onChange={(event) => setMinStock(event.target.value)} /></div></Section>
...
export function StockMovementModal({ open, product, onClose }: { open: boolean; product?: ProductRecord; onClose: () => void }) {
  ...
  const save = () => {
    ...
    if (type !== "Ajuste" && parsedQuantity <= 0) {
      toastInfo("Quantidade invalida", "Informe uma quantidade maior que zero.");
      return;
    }
    adjustStock(product.id, type, parsedQuantity, reason.trim() || `${type} manual`);
    toastSuccess("Estoque atualizado com sucesso");
    onClose();
  };

  return (
    <Modal open={open} title="Movimentar estoque" description={product ? product.name : undefined} size="md" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>Salvar movimento</Button></>}>
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-2">
          {(["Entrada", "Saida", "Ajuste"] as const).map((option) => <button key={option} type="button" className={`rounded-md border px-3 py-3 text-sm font-medium ${type === option ? "border-noogym-lime bg-noogym-lime text-black" : "border-white/10 bg-white/[0.03] text-zinc-200"}`} onClick={() => setType(option)}>{option}</button>)}
        </div>
        <div className="grid grid-cols-2 gap-3">
          <FormInput label={type === "Ajuste" ? "Novo estoque" : "Quantidade"} type="number" min="0" value={quantity} onChange={(event) => setQuantity(event.target.value)} />
          <FormInput label="Estoque final" value={String(nextStock)} readOnly />
        </div>
        <FormTextarea label="Motivo" placeholder="Ex: reposicao, perda, conferencia..." value={reason} onChange={(event) => setReason(event.target.value)} />
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 text-sm text-zinc-300">
          Estoque atual: <strong className="text-white">{product?.stock ?? 0} {product?.unit ?? "un"}</strong>
        </div>
      </div>
    </Modal>
  );
...
const parseSaleAmount = (value: string, subtotal: number) => {
  ...
  if (normalized.endsWith("%")) {
    ...
    return Number.isFinite(percent) ? Math.max(0, subtotal * (percent / 100)) : 0;
  }
...
const classCategories = ["Cardio", "Funcional", "Corpo e Mente", "Danca", "Luta", "Musculacao", "Aulas"];
...
const classEquipmentOptions = ["Bike Spinning", "Tapete", "Halteres", "Elastico", "Step", "Bola", "Corda", "Colchonete", "Toalha", "Garrafa de agua"];
const equipmentList = (value: string) => value.split(",").map((item) => item.trim()).filter(Boolean);
const toggleEquipmentValue = (value: string, option: string) => {
  ...
  return exists ? items.filter((item) => item.toLowerCase() !== option.toLowerCase()).join(", ") : [...items, option].join(", ");
};

export function ClassSessionModal({ open, lesson, onClose }: { open: boolean; lesson?: ClassRecord; onClose: () => void }) {
  const addClass = useClassesStore((state) => state.addClass);
  const updateClass = useClassesStore((state) => state.updateClass);
  const employees = useEmployeesStore((state) => state.employees);
  const [name, setName] = useState("");
  ...
  const instructorOptions = useMemo(() => {
    const names = employees.filter((employee) => employee.status !== "Inativo").map((employee) => employee.name);
    return names.length ? names : ["Instrutor", "Joao Silva", "Lucas Ferreira", "Mariana Costa"];
  }, [employees]);
  ...
  return (
    <Modal open={open} title={lesson ? "Editar aula" : "Nova aula"} description="Cadastre o modelo e a sessao da aula." size="lg" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>{lesson ? "Salvar alteracoes" : "Salvar aula"}</Button></>}>
      <div className="space-y-5">
        <Section title="1. Dados da aula"><div className="grid grid-cols-2 gap-3"><FormInput label="Nome da aula" requiredMark value={name} onChange={(event) => setName(event.target.value)} /><FormSelect label="Categoria" requiredMark options={classCategories} value={category} onChange={(event) => setCategory(event.target.value)} /></div><FormTextarea label="Descricao" value={description} onChange={(event) => setDescription(event.target.value)} /><div className="grid grid-cols-3 gap-3"><FormSelect label="Duracao" requiredMark options={["30 min", "45 min", "55 min", "60 min", "75 min", "90 min"]} value={duration} onChange={(event) => setDuration(event.target.value)} /><FormInput label="Capacidade" requiredMark type="number" min="1" max="200" value={seats} onChange={(event) => setSeats(event.target.value)} /><FormSelect label="Nivel" options={["Todos os niveis", "Iniciante", "Intermediario", "Avancado"]} value={level} onChange={(event) => setLevel(event.target.value)} /></div></Section>
        <Section title="2. Agenda e equipa"><div className="grid grid-cols-2 gap-3"><FormInput label="Data e hora" type="datetime-local" requiredMark value={startAt} onChange={(event) => setStartAt(event.target.value)} /><FormSelect label="Instrutor" requiredMark options={instructorOptions} value={instructor} onChange={(event) => setInstructor(event.target.value)} /><FormInput label="Sala" placeholder="Ex: Sala 1, Estudio A, Outdoor" value={room} onChange={(event) => setRoom(event.target.value)} /><FormSelect label="Modalidade" options={["Coletiva", "Individual", "Online", "Aula avulsa"]} value={modality} onChange={(event) => setModality(event.target.value)} /></div></Section>
        <Section title="3. Equipamentos e cor"><div className="space-y-3"><span className="block text-sm text-zinc-200">Equipamentos necessarios</span><div className="flex flex-wrap gap-2">{classEquipmentOptions.map((option) => { const selected = equipmentList(equipment).some((item) => item.toLowerCase() === option.toLowerCase()); return <button key={option} type="button" onClick={() => setEquipment((value) => toggleEquipmentValue(value, option))} className={`rounded border px-3 py-2 text-sm ${selected ? "border-noogym-lime bg-noogym-lime/10 text-noogym-lime" : "border-white/10 bg-white/[0.03] text-zinc-200"}`}>{option}</button>; })}</div><FormInput label="Outros equipamentos" placeholder="Digite outro equipamento e separe por virgula" value={equipment} onChange={(event) => setEquipment(event.target.value)} /></div><div className="space-y-3"><div className="flex items-center justify-between text-sm text-zinc-400"><span>Cor da aula</span><span className="inline-flex items-center gap-2 text-zinc-200"><span className="h-3 w-3 rounded-full" style={{ backgroundColor: color }} />{color}</span></div><ColorPicker value={color} onChange={setColor} /></div></Section>
        <Section title="4. Configuracoes"><div className="grid grid-cols-3 gap-3"><FormSwitch label="Aula ativa" checked={active} onChange={setActive} /><FormSwitch label="Permitir lista de espera" checked={waitingList} onChange={setWaitingList} /><FormSwitch label="Exige check-in" checked={requiresCheckin} onChange={setRequiresCheckin} /></div></Section>
      </div>
    </Modal>
  );
}

export function ClassAgendaModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const baseLesson = classes.find((lesson) => lesson.id === lessonId) ?? classes[0];
  const instructorOptions = useMemo(() => {
    const names = employees.filter((employee) => employee.status !== "Inativo").map((employee) => employee.name);
    return names.length ? names : ["Joao Silva", "Lucas Ferreira", "Mariana Costa"];
  }, [employees]);

  useEffect(() => {
    if (!open) return;
    setLessonId(classes[0]?.id ?? "");
    setInstructor(classes[0]?.instructor ?? instructorOptions[0] ?? "Instrutor");
    setSelectedSlots([]);
  }, [classes, instructorOptions, open]);

  const toggleSlot = (slot: string) => setSelectedSlots((slots) => slots.includes(slot) ? slots.filter((item) => item !== slot) : [...slots, slot]);
  ...
  const save = () => {
    ...
    selectedSlots.forEach((slot) => {
      ...
      addClass({
        ...
        endAtIso: classEndFromStart(classDateTimeInputValue(start.toISOString()), duration)
      });
    });
...
export function ClassRosterModal({ open, lesson, onClose }: { open: boolean; lesson?: ClassRecord; onClose: () => void }) {
  const clients = useClientsStore((state) => state.clients);
  const updateParticipants = useClassesStore((state) => state.updateParticipants);
  ...
  const filteredRoster = useMemo(() => {
    ...
    return roster.filter((client) => {
      ...
      const matchesFilter = filter === "Todos" || (filter === "Presentes" ? present : !present);
      const text = `${client.name} ${client.phone} ${client.email} ${client.document ?? ""} ${client.plan}`.toLowerCase();
  ...
  useEffect(() => {
    ...
    setQuery("");
    setFilter("Todos");
    setPresentIds(roster.slice(0, lesson?.participants ?? 0).map((client) => client.id));
  }, [lesson?.participants, open, roster]);

  const toggle = (id: string) => setPresentIds((ids) => ids.includes(id) ? ids.filter((item) => item !== id) : [...ids, id]);
  const markVisible = (present: boolean) => setPresentIds((ids) => {
    const visibleIds = filteredRoster.map((client) => client.id);
    return present ? Array.from(new Set([...ids, ...visibleIds])) : ids.filter((id) => !visibleIds.includes(id));
  });
  const save = () => {
  ...
  return (
    <Modal open={open} title="Participantes da aula" description={lesson ? `${lesson.name} - ${lesson.time}` : undefined} size="lg" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>Salvar presencas</Button></>}>
      <div className="space-y-4">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_160px]">
          <FormInput label="Pesquisar aluno" placeholder="Nome, telefone, email, BI ou plano" value={query} onChange={(event) => setQuery(event.target.value)} />
          <FormSelect label="Filtro" options={["Todos", "Presentes", "Ausentes"]} value={filter} onChange={(event) => setFilter(event.target.value as "Todos" | "Presentes" | "Ausentes")} />
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/[0.03] p-3 text-sm">
          <span><strong className="text-noogym-lime">{presentIds.length}</strong> presentes de {roster.length} alunos</span>
          <span>{filteredRoster.length} resultados visiveis</span>
          <div className="flex gap-2"><Button className="h-8" onClick={() => markVisible(true)}>Marcar visiveis</Button><Button className="h-8" onClick={() => markVisible(false)}>Limpar visiveis</Button></div>
        </div>
        <div className="max-h-[440px] space-y-2 overflow-auto pr-1">
          {filteredRoster.length ? filteredRoster.map((client) => {
            const present = presentIds.includes(client.id);
            return (
              <button key={client.id} type="button" onClick={() => toggle(client.id)} className={`flex w-full items-center gap-3 rounded-md border p-3 text-left transition ${present ? "border-noogym-lime/60 bg-noogym-lime/10" : "border-white/10 bg-white/[0.025] hover:border-white/20"}`}>
                <Avatar label={client.avatar ?? "CL"} />
                <span className="min-w-0 flex-1"><span className="block truncate font-medium">{client.name}</span><span className="block truncate text-xs text-zinc-400">{client.phone} - {client.plan}</span></span>
                <span className={`rounded border px-3 py-1 text-xs ${present ? "border-noogym-lime/60 text-noogym-lime" : "border-white/10 text-zinc-400"}`}>{present ? "Presente" : "Ausente"}</span>
              </button>
            );
          }) : <p className="rounded-lg border border-white/10 p-6 text-center text-sm text-zinc-400">Nenhum aluno encontrado para esta pesquisa.</p>}
        </div>
      </div>
    </Modal>
  );
...
export function WorkoutBuilderModal({ open, workout, onClose }: { open: boolean; workout?: WorkoutRecord; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setAuthor(workout?.author ?? employees.find((employee) => employee.role.includes("Personal"))?.name ?? "Admin");
    setStatus(workout?.status ?? "Ativo");
    setNotes(workout?.notes ?? "");
    setBlockName(firstBlock?.name ?? "Treino principal");
    setExerciseRows(firstBlock?.exercises?.length ? firstBlock.exercises : [
      { id: "draft-1", name: "Agachamento livre", group: "Pernas", equipment: "Barra", sets: 4, reps: "10-12", load: "Moderada", rest: "75s", notes: "" },
      { id: "draft-2", name: "Supino reto", group: "Peito", equipment: "Barra", sets: 4, reps: "8-10", load: "Progressiva", rest: "90s", notes: "" }
    ]);
  }, [clients, employees, open, workout]);

  const updateExercise = (id: string, patch: Partial<WorkoutExerciseRecord>) => {
    setExerciseRows((rows) => rows.map((row) => row.id === id ? { ...row, ...patch } : row));
  };

  const addExerciseRow = () => {
    setExerciseRows((rows) => [...rows, { id: `draft-${Date.now()}`, name: "", group: "Geral", equipment: "Livre", sets: 3, reps: "10", load: "", rest: "60s", notes: "" }]);
  };
  ...
  const save = () => {
    ...
    const payload = {
      ...
      blocks: [{ id: workout?.blocks?.[0]?.id ?? `block-${Date.now()}`, name: blockName.trim() || "Treino principal", exercises: validRows }],
  ...
  return (
    <Modal open={open} title={workout ? "Editar treino" : "Novo treino"} description="Monte a ficha com aluno, objetivo, revisao e exercicios." size="xl" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>Salvar treino</Button></>}>
      <div className="space-y-5">
        <Section title="1. Dados do treino">
          <div className="grid gap-3 md:grid-cols-2">
            <FormInput label="Nome do treino" value={name} onChange={(event) => setName(event.target.value)} />
            <FormSelect label="Cliente" options={["Modelo sem cliente", ...clients.map((item) => item.name)]} value={client} onChange={(event) => setClient(event.target.value)} />
            <FormSelect label="Objetivo" options={["Hipertrofia", "Emagrecimento", "Forca", "Condicionamento", "Definicao", "Reabilitacao"]} value={goal} onChange={(event) => setGoal(event.target.value)} />
            <FormSelect label="Nivel" options={["Iniciante", "Intermediario", "Avancado"]} value={level} onChange={(event) => setLevel(event.target.value)} />
            <FormSelect label="Frequencia" options={["2x por semana", "3x por semana", "4x por semana", "5x por semana", "Livre"]} value={frequency} onChange={(event) => setFrequency(event.target.value)} />
            <FormSelect label="Revisao" options={["15 dias", "30 dias", "45 dias", "60 dias"]} value={reviewDate} onChange={(event) => setReviewDate(event.target.value)} />
            <FormInput label="Duracao media" value={duration} onChange={(event) => setDuration(event.target.value)} />
            <FormSelect label="Criado por" options={["Admin", ...employees.map((employee) => employee.name)]} value={author} onChange={(event) => setAuthor(event.target.value)} />
          </div>
        </Section>
        <Section title="2. Exercicios">
          <FormInput label="Bloco" value={blockName} onChange={(event) => setBlockName(event.target.value)} />
          <div className="space-y-3">
            {exerciseRows.map((row, index) => (
              <div key={row.id} className="rounded-lg border border-white/10 bg-white/[0.02] p-3">
                <div className="mb-3 flex items-center justify-between">
                  <strong className="text-sm text-white">Exercicio {index + 1}</strong>
                  <button type="button" className="text-xs text-red-300" onClick={() => removeExercise(row.id)}>Remover</button>
                </div>
                <div className="grid gap-3 md:grid-cols-[minmax(0,1.4fr)_120px_120px_120px_120px]">
                  <FormInput label="Nome" value={row.name} onChange={(event) => updateExercise(row.id, { name: event.target.value })} />
                  <FormSelect label="Grupo" options={["Geral", "Peito", "Costas", "Pernas", "Ombros", "Bracos", "Core", "Cardio"]} value={row.group} onChange={(event) => updateExercise(row.id, { group: event.target.value })} />
                  <FormInput label="Series" type="number" value={String(row.sets)} onChange={(event) => updateExercise(row.id, { sets: Number(event.target.value) || 1 })} />
                  <FormInput label="Reps" value={row.reps} onChange={(event) => updateExercise(row.id, { reps: event.target.value })} />
                  <FormInput label="Descanso" value={row.rest} onChange={(event) => updateExercise(row.id, { rest: event.target.value })} />
                </div>
                <div className="mt-3 grid gap-3 md:grid-cols-3">
                  <FormInput label="Equipamento" value={row.equipment} onChange={(event) => updateExercise(row.id, { equipment: event.target.value })} />
                  <FormInput label="Carga" value={row.load ?? ""} onChange={(event) => updateExercise(row.id, { load: event.target.value })} />
                  <FormInput label="Notas" value={row.notes ?? ""} onChange={(event) => updateExercise(row.id, { notes: event.target.value })} />
                </div>
              </div>
            ))}
          </div>
          <Button icon={<Dumbbell className="h-4 w-4" />} onClick={addExerciseRow}>Adicionar exercicio</Button>
        </Section>
        <div className="grid gap-3 md:grid-cols-[1fr_180px]">
          <FormTextarea label="Observacoes" value={notes} onChange={(event) => setNotes(event.target.value)} />
          <FormSelect label="Status" options={["Ativo", "Rascunho", "Pausado", "Arquivado"]} value={status} onChange={(event) => setStatus(event.target.value)} />
        </div>
      </div>
    </Modal>
  );
...
export function EmployeeBuilderModal({ open, employee, onClose }: { open: boolean; employee?: EmployeeRecord; onClose: () => void }) {
  const addEmployee = useEmployeesStore((state) => state.addEmployee);
  const updateEmployee = useEmployeesStore((state) => state.updateEmployee);
  const roles = useEmployeesStore((state) => state.roles);
  ...
  const [shift, setShift] = useState("Manha");
  const [accessStatus, setAccessStatus] = useState("Liberado");
  const [accountMode, setAccountMode] = useState<EmployeeRecord["accountMode"]>("Sem acesso");
  const [accountEmail, setAccountEmail] = useState("");
  const [userId, setUserId] = useState("");
  const [gymScope, setGymScope] = useState<EmployeeRecord["gymScope"]>("Organizacao");
  const [gymIds, setGymIds] = useState<string[]>([]);
  const [salary, setSalary] = useState("");
  const [status, setStatus] = useState("Ativo");
  const [notes, setNotes] = useState("");
  const selectedRole = roles.find((item) => item.name === role);
  const gymOptions = gyms.length ? gyms : [{ id: "unidade-central", name: "Unidade Central" }, { id: "multiunidade", name: "Todas as unidades" }];
  ...
  useEffect(() => {
    ...
    setShift(employee?.shift ?? "Manha");
    setAccessStatus(employee?.accessStatus ?? "Liberado");
    setAccountMode(employee?.accountMode ?? (employee?.userId ? "Vincular usuario existente" : "Sem acesso"));
    setAccountEmail(employee?.accountEmail ?? employee?.email ?? "");
    setUserId(employee?.userId ?? "");
    setGymScope(employee?.gymScope ?? (employee?.role?.includes("Personal") ? "Multiunidade" : "Organizacao"));
    setGymIds(employee?.gymIds?.length ? employee.gymIds : employee?.gymId ? [employee.gymId] : []);
    setSalary(employee?.salary ?? "");
    setStatus(employee?.status ?? "Ativo");
    setNotes(employee?.notes ?? "");
  }, [employee, open, roles]);
  ...
  useEffect(() => {
    if (accountMode === "Convidar nova conta" && !accountEmail) setAccountEmail(email);
  }, [accountEmail, accountMode, email]);

  const toggleGym = (id: string) => {
    setGymIds((ids) => ids.includes(id) ? ids.filter((item) => item !== id) : [...ids, id]);
  };
  ...
  const save = () => {
    ...
    if (accountMode === "Convidar nova conta" && !accountEmail.trim()) {
      toastInfo("Email de acesso obrigatorio", "Informe o email para enviar o convite.");
      return;
    }
    if (accountMode === "Vincular usuario existente" && !userId.trim()) {
      toastInfo("Usuario obrigatorio", "Selecione ou informe o usuario existente.");
      return;
    }
    const resolvedGymIds = gymScope === "Organizacao" ? [] : gymScope === "Unidade especifica" ? gymIds.slice(0, 1) : gymIds.length ? gymIds : gymOptions.map((gym) => gym.id);
    const resolvedAccessStatus = accountMode === "Sem acesso" ? "Sem acesso" : accountMode === "Convidar nova conta" ? "Convite pendente" : accessStatus === "Sem acesso" ? "Liberado" : accessStatus;
    const payload = {
      name: name.trim(),
      role,
      email: email.trim(),
      phone: phone.trim(),
      userId: accountMode === "Vincular usuario existente" ? userId.trim() : undefined,
      ...
      accountMode,
      accountEmail: accountMode === "Sem acesso" ? undefined : accountEmail.trim(),
      accountStatus: accountMode === "Sem acesso" ? "Sem conta" : accountMode === "Convidar nova conta" ? "Convite pendente" : "Conta vinculada",
      gymScope,
    ...
    employee ? updateEmployee(employee.id, payload) : addEmployee(payload);
    toastSuccess(employee ? "Funcionario atualizado com sucesso" : "Funcionario criado com sucesso");
    onClose();
  };

  return (
    <Modal open={open} title={employee ? "Editar funcionario" : "Novo funcionario"} description="Cadastre dados profissionais, acesso e permissao." size="xl" onClose={onClose} footer={<><Button onClick={onClose}>Cancelar</Button><Button variant="primary" onClick={save}>Salvar funcionario</Button></>}>
      <div className="space-y-5">
        <Section title="1. Dados pessoais">
          <div className="grid gap-3 md:grid-cols-[180px_1fr_1fr]">
            <FileUpload label="Foto opcional" />
            <FormInput label="Nome" value={name} onChange={(event) => setName(event.target.value)} />
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <FormInput label="Data de admissao" type="date" value={hireDate} onChange={(event) => setHireDate(event.target.value)} />
            <FormInput label="Salario mensal" value={salary} onChange={(event) => setSalary(event.target.value)} />
          </div>
        </Section>
        <Section title="2. Dados profissionais">
          <div className="grid gap-3 md:grid-cols-3">
            <FormSelect label="Funcao" options={roles.map((item) => item.name)} value={role} onChange={(event) => setRole(event.target.value)} />
            <FormSelect label="Departamento" options={["Administracao", "Atendimento", "Tecnico", "Financeiro", "Comercial"]} value={department} onChange={(event) => setDepartment(event.target.value)} />
            <FormSelect label="Contrato" options={["Tempo integral", "Meio periodo", "Prestador", "Estagiario"]} value={contractType} onChange={(event) => setContractType(event.target.value)} />
            <FormInput label="Supervisor" value={supervisor} onChange={(event) => setSupervisor(event.target.value)} />
            <FormSelect label="Turno" options={["Manha", "Tarde", "Noite", "Rotativo"]} value={shift} onChange={(event) => setShift(event.target.value)} />
            <FormSelect label="Status" options={["Ativo", "Inativo", "Licenca", "Desligado"]} value={status} onChange={(event) => setStatus(event.target.value)} />
          </div>
        </Section>
        <Section title="3. Acesso e permissoes">
          <div className="grid gap-3 lg:grid-cols-[280px_1fr]">
            <div className="space-y-3">
              <FormSelect label="Conta de acesso" options={["Sem acesso", "Convidar nova conta", "Vincular usuario existente"]} value={accountMode} onChange={(event) => setAccountMode(event.target.value as EmployeeRecord["accountMode"])} />
              {accountMode === "Convidar nova conta" ? <FormInput label="Email do convite" type="email" value={accountEmail} onChange={(event) => setAccountEmail(event.target.value)} /> : null}
              {accountMode === "Vincular usuario existente" ? (
                userOptions.length ? <FormSelect label="Usuario existente" value={userId} onChange={(event) => setUserId(event.target.value)}>
                  <option value="">Selecione</option>
                  {userOptions.map((user) => <option key={user.id} value={user.id}>{user.name} - {user.email}</option>)}
                </FormSelect> : <FormInput label="ID do usuario existente" value={userId} onChange={(event) => setUserId(event.target.value)} />
              ) : null}
              {accountMode !== "Sem acesso" ? <FormSelect label="Status de acesso" options={["Liberado", "Bloqueado", "Convite pendente"]} value={accessStatus} onChange={(event) => setAccessStatus(event.target.value)} /> : null}
            </div>
            <div className="space-y-3">
              <FormSelect label="Escopo de unidade" options={["Organizacao", "Unidade especifica", "Multiunidade"]} value={gymScope} onChange={(event) => setGymScope(event.target.value as EmployeeRecord["gymScope"])} />
              {gymScope !== "Organizacao" ? (
                <div className="grid gap-2 md:grid-cols-2">
                  {gymOptions.map((gym) => <FormCheckbox key={gym.id} label={gym.name} checked={gymIds.includes(gym.id)} onChange={() => toggleGym(gym.id)} />)}
                </div>
              ) : <p className="rounded-md border border-white/10 bg-white/[0.03] p-3 text-sm text-zinc-400">A conta pode operar no contexto da organizacao. A senha continua pertencendo ao usuario, nao ao cadastro do funcionario.</p>}
              <div className="grid gap-2 md:grid-cols-3">
                {(selectedRole?.modules ?? []).map((permission) => <FormCheckbox key={permission} label={permission} checked readOnly />)}
                {!selectedRole?.modules.length ? <p className="rounded-md border border-white/10 p-3 text-sm text-zinc-400">Funcao sem permissoes configuradas.</p> : null}
              </div>
            </div>
          </div>
        </Section>
        <FormTextarea label="Observacoes internas" value={notes} onChange={(event) => setNotes(event.target.value)} />
      </div>
    </Modal>
  );
}
