  ...
  return (
    <div className="mb-3 flex min-w-0 flex-wrap gap-x-4 gap-y-2 text-xs text-zinc-300">
      {series.map((item, index) => (
        <span key={`${item.name}-${index}`} className="flex min-w-0 items-center gap-2">
          ...
          return (
            <g key={`${item.name}-${seriesIndex}`}>
              ...
              {points.map((point, pointIndex) => {
                ...
                return <circle key={`${item.name}-${seriesIndex}-${pointIndex}`} cx={x} cy={y} r="1.15" fill={item.color} />;
              })}
            </g>
          );
        })}
      ...
      {labels ? (
        <div className="mt-2 grid grid-flow-col text-center text-[11px] text-zinc-500">
          {labels.map((label, index) => (
            <span key={`${label}-${index}`} className="min-w-0 truncate px-0.5">{label}</span>
          ))}
        </div>
      ) : null}
    </div>
  ...
  return (
    <div className="flex min-w-0 items-end gap-3 px-1 sm:gap-5 sm:px-2" style={{ height }}>
      {values.map((value, index) => (
        <div key={`${labels[index] ?? "bar"}-${index}`} className="flex min-w-0 flex-1 flex-col items-center gap-2">
          ...
          <div key={`${label}-${index}`} className="flex min-w-0 flex-1 flex-col items-center gap-2">
            <div className="flex h-[150px] items-end gap-1">
              {groups.map((group, groupIndex) => (
                <span key={`${group.name}-${groupIndex}`} className="block w-4 rounded-t" style={{ height: `${Math.max(8, (group.values[index] / max) * 145)}px`, backgroundColor: group.color }} />
              ))}
            </div>
    ...
    <div className="flex min-w-0 flex-col items-center gap-5 sm:flex-row sm:items-center">
      ...
      <div className="w-full min-w-0 flex-1 space-y-3 text-sm">
        {chartItems.map((item, index) => (
          <div key={`${item.label}-${index}`} className="flex items-start justify-between gap-4">
          ...
          <div key={`${label}-${index}`} className="flex min-w-0 flex-1 flex-col items-center gap-2">
            <div className="flex w-full max-w-8 flex-col-reverse overflow-hidden rounded-t" style={{ height: `${Math.max(24, (totals[index] / max) * 170)}px` }}>
              {series.map((item, itemIndex) => (
                <span key={`${item.name}-${itemIndex}`} className="block w-full" style={{ height: `${totals[index] > 0 ? (item.values[index] / totals[index]) * 100 : 0}%`, backgroundColor: item.color }} />
              ))}
            </div>
