...
export default function Configuracoes() {
  ...
  const configDescription = onlineOnly ? "Configuracao online preparada para integracao com a API do servidor." : "Configuracao local-first preparada para futura integracao com SQLite.";

  return (
    <div className="page-grid">
      <div className="panel p-6">
        <PageHeader title="Configuracoes" subtitle="Gerencie as configuracoes do sistema e da sua academia." />
            ...
            <Card key={item} className="p-5">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-3 text-sm text-zinc-400">{configDescription}</p>
                  {index % 2 === 0 ? (
                  ...
                  <Button className="mt-5 min-w-32" onClick={() => setModalTitle(`${tab}: ${item}`)}>
                    Editar
                  </Button>
                </div>
              </div>
      ...
      <aside className="space-y-3">
        <Card className="p-6">
          ...
          <div className="mt-8 flex items-center gap-6">
            ...
            <Button onClick={() => setModalTitle("Academia: Logo")}>Alterar logo</Button>
          </div>
          <h3 className="mt-6 text-xl font-semibold">Noogym Fitness Center</h3>
          <div className="mt-4 space-y-3 text-sm text-zinc-300">
            <p>Unidade Central</p>
            <p>NIF: 5001234567</p>
            <p>Avenida 21 de Janeiro, Luanda, Angola</p>
            <p>+244 923 777 888</p>
            <p>contato@noogym.com</p>
          </div>
        </Card>
        <Card className="p-6">
          ...
          <Button className="w-full" onClick={() => setModalTitle("Restaurar padroes")}>
            Restaurar padroes
          </Button>
        </Card>
        <Card className="p-5">
          ...
          <p className="mt-3 text-sm text-zinc-400">
            {onlineOnly
              ? "Dados operacionais sincronizados com o servidor na versao web."
              : "Dados operacionais persistem localmente e ficam prontos para sincronizacao posterior."}
          </p>
        </Card>
      </aside>
      <SettingsModal open={Boolean(modalTitle)} title={modalTitle ?? "Configuracoes"} onClose={() => setModalTitle(null)} />
    </div>
  );
}
