...
const defaultRoles: EmployeeRoleRecord[] = [
  { id: "ROLE-OWNER", name: "Proprietario", description: "Dono da organizacao com acesso total.", modules: employeeModules, status: "Ativo", employees: 0 },
...
const defaultDepartment = (employee: EmployeeRecord) => employee.role.includes("Personal") || employee.role.includes("Instrutor") ? "Tecnico" : employee.role === "Recepcionista" ? "Atendimento" : employee.role === "Proprietario" ? "Direcao" : "Administracao";
...
const mergeDefaultRoles = (storedRoles: EmployeeRoleRecord[]) => {
  ...
  return [
    ...defaultRoles.map((role) => storedByName.get(role.name) ? { ...role, ...storedByName.get(role.name), modules: storedByName.get(role.name)?.modules?.length ? storedByName.get(role.name)!.modules : role.modules } : role),
    ...storedRoles.filter((role) => !defaultRoles.some((item) => item.name === role.name))
  ];
};
...
const shouldShowUserAsEmployee = (user: UserSettings, activeGymId?: string) => {
  ...
  return gymIds.includes(activeGymId) || user.role === "OWNER" || user.role === "SUPER_ADMIN";
};
const userToEmployee = (user: UserSettings, index = 0): EmployeeRecord => normalizeEmployee({
  id: `USER-${user.id}`,
  userId: user.id,
  name: user.name,
  ...
  accountStatus: user.status === "ACTIVE" ? "Conta vinculada" : userAccessStatusLabel(user.status),
  gymScope: userGymIds(user).length > 1 ? "Multiunidade" : userGymIds(user).length === 1 ? "Unidade especifica" : "Organizacao",
  gymIds: userGymIds(user),
  lastAccess: user.lastLoginAt ? new Intl.DateTimeFormat("pt-AO", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(user.lastLoginAt)) : index % 2 === 0 ? "Sem acesso recente" : "Hoje, 09:30",
  permissions: roleModules(roleLabel(user.role)),
  notes: "Usuario administrativo sem cadastro de funcionario operacional."
});
const mergeUsersWithoutEmployeeProfiles = (employees: EmployeeRecord[], users: UserSettings[], activeGymId?: string) => {
  const employeeUserIds = new Set(employees.map((employee) => employee.userId).filter(Boolean));
  const employeeEmails = new Set(employees.map((employee) => employee.email.toLowerCase()));
  const userEmployees = users
    .filter((user) => shouldShowUserAsEmployee(user, activeGymId))
    .filter((user) => !employeeUserIds.has(user.id) && !employeeEmails.has(user.email.toLowerCase()))
    .map(userToEmployee);
...
export const useEmployeesStore = create<{
  ...
  loadOnline: async () => {
    ...
    const syncedEmployees = apiEmployees.map(employeeFromApi).map((employee, index) => {
  ...
  addEmployee: (employee) => set((state) => {
    const created: EmployeeRecord = normalizeEmployee({ id: uid("FUNC"), gymId: useAppStore.getState().activeGymId ?? undefined, name: "Novo funcionario", role: "Recepcionista", email: "funcionario@noogym.com", phone: "+244 900 000 000", status: "Ativo", salary: "0 Kz", ...employee } as EmployeeRecord);
