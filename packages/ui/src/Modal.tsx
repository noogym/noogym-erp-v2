  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-3 backdrop-blur-sm sm:p-5">
      <section className={`panel flex max-h-[88dvh] w-full ${sizes[size]} flex-col overflow-hidden`}>
        <header className="flex items-start gap-4 border-b border-white/10 p-4 sm:p-5">
        ...
        {footer ? <footer className="grid gap-3 border-t border-white/10 p-4 sm:grid-cols-2 sm:p-5">{footer}</footer> : null}
      </section>
    </div>
  );
}
