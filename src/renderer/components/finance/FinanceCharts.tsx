...
export function Legend({ series }: { series: Array<{ name: string; color: string }> }) {
  return (
    <div className="mb-3 flex flex-wrap gap-4 text-xs text-zinc-300">
      {series.map((item) => (
...
export function LineChart({ series, labels, height = 220 }: { series: FinanceSeries[]; labels?: string[]; height?: number }) {
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 100 72" preserveAspectRatio="none" className="w-full" style={{ height }}>
        {[14, 28, 42, 56].map((y) => (
...
export function BarChart({
  ...
  return (
    <div className="flex items-end gap-5 px-2" style={{ height }}>
      {values.map((value, index) => (
        <div key={labels[index]} className="flex min-w-0 flex-1 flex-col items-center gap-2">
          <span className="whitespace-nowrap text-xs text-zinc-100">{value.toLocaleString("pt-AO")}</span>
